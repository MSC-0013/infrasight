...
import { _ as useAuthStore, l as StatusBadge, R as ROLE_LABEL, U as generateTimeSeries, D as Dialog, a as DialogContent, c as DialogHeader, d as DialogTitle, b as DialogDescription, p as cn, Q as generateThroughputSeries, E as generateLatencySeries, T as Tabs, n as TabsList, o as TabsTrigger, m as TabsContent, A as generateEvents, t as generateAlerts, Y as generateWorkers, L as generateQueues, K as generateQueueLagSeries, z as generateEventDistribution, V as generateTimelineEvents, J as JSONViewer, $ as useInspector, q as formatDistanceToNow$1 } from "./router-F21aWjaR.mjs";
...
import { g as ChartColumn, B as Bell, _ as Sparkles, a7 as Workflow, F as FileText, O as OctagonAlert, G as GitBranch, N as Network, Z as ShieldCheck, A as Activity, f as Building2, K as KeyRound, S as Plus, Y as Settings, a6 as Users, s as Flame, y as ListFilter, T as RefreshCw, D as Download, k as ChevronRight, r as Cpu, L as Layers, a3 as TriangleAlert, U as Rocket, l as ChevronUp, i as ChevronDown } from "../_libs/lucide-react.mjs";
...
function createSupabaseClient() {
  const SUPABASE_URL = "https://stxaxicgfdighpacyphd.supabase.co";
  const SUPABASE_PUBLISHABLE_KEY = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InV6aWt5ZW51Y3plcG1idWR3dnJvIiwicm9sZSI6ImFub24iLCJpYXQiOjE3Nzg4NzkzMzEsImV4cCI6MjA5NDQ1NTMzMX0.___MuQGOpv0ohvAlAVB5QTtI1or_alURk3D4Y6QiDDg";
...
async function fetchDashboardMetrics() {
  ...
  const { count: eventsCount } = await supabase.from("events").select("*", { count: "exact", head: true }).eq("org_id", orgId).gte("timestamp", oneHourAgo);
  const { count: activeAlerts } = await supabase.from("alerts").select("*", { count: "exact", head: true }).eq("org_id", orgId).eq("acknowledged", false);
  const { count: onlineWorkers } = await supabase.from("workers").select("*", { count: "exact", head: true }).eq("org_id", orgId).eq("status", "online");
  const { count: totalWorkers } = await supabase.from("workers").select("*", { count: "exact", head: true }).eq("org_id", orgId);
  const { count: openIncidents } = await supabase.from("incidents").select("*", { count: "exact", head: true }).eq("org_id", orgId).neq("status", "resolved");
  const { count: healthyServices } = await supabase.from("services").select("*", { count: "exact", head: true }).eq("org_id", orgId).eq("status", "healthy");
  const { count: totalServices } = await supabase.from("services").select("*", { count: "exact", head: true }).eq("org_id", orgId);
  const { data: svcData } = await supabase.from("services").select("p95_ms, error_rate, rps, uptime_pct").eq("org_id", orgId);
  const avgP95 = svcData?.length ? Math.round(svcData.reduce((a, s) => a + s.p95_ms, 0) / svcData.length) : 0;
  const avgErrorRate = svcData?.length ? +(svcData.reduce((a, s) => a + s.error_rate, 0) / svcData.length).toFixed(2) : 0;
  const totalRps = svcData?.reduce((a, s) => a + Number(s.rps), 0) ?? 0;
  const avgUptime = svcData?.length ? +(svcData.reduce((a, s) => a + s.uptime_pct, 0) / svcData.length).toFixed(3) : 0;
  const { data: queuesData } = await supabase.from("queues").select("lag_ms, status").eq("org_id", orgId);
  const avgLag = queuesData?.length ? Math.round(queuesData.reduce((a, q) => a + q.lag_ms, 0) / queuesData.length) : 0;
  return {
...
async function fetchSuperAdminMetrics() {
  ...
  const { count: totalApiKeys } = await supabase.from("api_keys").select("*", { count: "exact", head: true }).eq("status", "active");
  const { count: auditToday } = await supabase.from("audit_logs").select("*", { count: "exact", head: true }).gte("timestamp", new Date(Date.now() - 864e5).toISOString());
  return {
...
async function fetchSREMetrics() {
  const orgId = await getUserOrgId();
  if (!orgId) return null;
  const { count: sev1 } = await supabase.from("incidents").select("*", { count: "exact", head: true }).eq("org_id", orgId).eq("severity", "sev1").neq("status", "resolved");
  const { data: slos } = await supabase.from("slos").select("budget_remaining, burn_rate, status").eq("org_id", orgId);
  const avgBudget = slos?.length ? +(slos.reduce((a, s) => a + Number(s.budget_remaining), 0) / slos.length).toFixed(1) : 100;
  const { count: deploys24h } = await supabase.from("deployments").select("*", { count: "exact", head: true }).eq("org_id", orgId).gte("started_at", new Date(Date.now() - 864e5).toISOString());
  return {
...
async function fetchDeveloperMetrics() {
  const orgId = await getUserOrgId();
  if (!orgId) return null;
  const { data: services } = await supabase.from("services").select("p95_ms, error_rate, status").eq("org_id", orgId);
  const { count: myIncidents } = await supabase.from("incidents").select("*", { count: "exact", head: true }).eq("org_id", orgId).eq("status", "investigating");
  return {
...
async function fetchViewerMetrics() {
  const orgId = await getUserOrgId();
  if (!orgId) return null;
  const { count: healthySvc } = await supabase.from("services").select("*", { count: "exact", head: true }).eq("org_id", orgId).eq("status", "healthy");
  const { count: totalSvc } = await supabase.from("services").select("*", { count: "exact", head: true }).eq("org_id", orgId);
  const { count: alerts } = await supabase.from("alerts").select("*", { count: "exact", head: true }).eq("org_id", orgId).eq("acknowledged", false);
  const { data: svcData } = await supabase.from("services").select("uptime_pct, rps").eq("org_id", orgId);
  return {
...
async function fetchRecentEvents(limit = 30) {
  const orgId = await getUserOrgId();
  if (!orgId) return [];
  const { data } = await supabase.from("events").select("*").eq("org_id", orgId).order("timestamp", { ascending: false }).limit(limit);
  return data ?? [];
}
async function fetchActiveAlerts(limit = 10) {
  const orgId = await getUserOrgId();
  if (!orgId) return [];
  const { data } = await supabase.from("alerts").select("*").eq("org_id", orgId).eq("acknowledged", false).order("timestamp", { ascending: false }).limit(limit);
  return data ?? [];
}
...
async function fetchIncidents() {
  const orgId = await getUserOrgId();
  if (!orgId) return [];
  const { data } = await supabase.from("incidents").select("*").eq("org_id", orgId).order("opened_at", { ascending: false });
  return data ?? [];
}
async function fetchDeployments(limit = 20) {
  const orgId = await getUserOrgId();
  if (!orgId) return [];
  const { data } = await supabase.from("deployments").select("*").eq("org_id", orgId).order("started_at", { ascending: false }).limit(limit);
  return data ?? [];
}
async function fetchSLOs() {
  const orgId = await getUserOrgId();
  if (!orgId) return [];
  const { data } = await supabase.from("slos").select("*, sli_metrics(*)").eq("org_id", orgId).order("name");
  return data ?? [];
}
...
async function fetchMembers() {
  const orgId = await getUserOrgId();
  if (!orgId) return [];
  const { data } = await supabase.from("members").select("*").eq("org_id", orgId).order("name");
  return data ?? [];
}
async function fetchAuditLogs(limit = 50) {
  const orgId = await getUserOrgId();
  if (!orgId) return [];
  const { data } = await supabase.from("audit_logs").select("*").eq("org_id", orgId).order("timestamp", { ascending: false }).limit(limit);
  return data ?? [];
}
...
async function fetchMLInsights() {
  const orgId = await getUserOrgId();
  if (!orgId) return [];
  const { data } = await supabase.from("ml_insights").select("*").eq("org_id", orgId).order("timestamp", { ascending: false });
  return data ?? [];
}
function SuperAdminDashboard() {
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex flex-col", children: [
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "grid grid-cols-2 gap-3 px-6 py-4 md:grid-cols-4 xl:grid-cols-8", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx(MetricCard, { label: "Workspaces", value: metrics?.totalOrgs ?? "—", series: sparkA, trend: 4.2, status: "info", variant: "area" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx(MetricCard, { label: "Total members", value: metrics?.totalMembers ?? "—", series: sparkB, trend: 6.1, status: "info" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx(MetricCard, { label: "Active API keys", value: metrics?.totalApiKeys ?? "—", series: sparkC, trend: 2.3, status: "success" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx(MetricCard, { label: "Audit 24h", value: metrics?.auditEvents24h ?? "—", series: sparkD, trend: -3.2, trendInverted: true, status: "info" }),
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "rounded-lg border border-border bg-card", children: [
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "max-h-[340px] overflow-auto", children: /* @__PURE__ */ jsxRuntimeExports.jsxs(Table, { children: [
          ...
          /* @__PURE__ */ jsxRuntimeExports.jsx(TableBody, { children: orgs.slice(0, 8).map((o) => /* @__PURE__ */ jsxRuntimeExports.jsxs(TableRow, { className: "cursor-pointer border-border text-xs hover:bg-accent/40", children: [
            ...
            /* @__PURE__ */ jsxRuntimeExports.jsx(TableCell, { className: "py-1.5 font-mono text-[11px] text-muted-foreground", children: o.region })
          ] }, o.id)) })
        ] }) })
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "rounded-lg border border-border bg-card", children: [
...
function AdminDashboard() {
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex flex-col", children: [
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "grid grid-cols-2 gap-3 px-6 py-4 md:grid-cols-4 xl:grid-cols-8", children: [
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsx(MetricCard, { label: "Active alerts", value: metrics?.activeAlerts ?? 0, series: generateTimeSeries(20, 4, 2), trend: 0, status: "warning" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx(MetricCard, { label: "Open incidents", value: metrics?.openIncidents ?? 0, series: generateTimeSeries(20, 3, 1), trend: -14, trendInverted: true, status: "info" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx(MetricCard, { label: "Events / hour", value: metrics?.eventsPerHour ?? 0, series: generateTimeSeries(20, 3200, 400), trend: 2.8, status: "info", variant: "area" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx(MetricCard, { label: "Uptime", value: `${metrics?.avgUptime ?? 99.9}%`, series: generateTimeSeries(20, 99.9, 8e-3), trend: 0.01, status: "success" })
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "grid grid-cols-1 gap-3 px-6 pt-3 lg:grid-cols-2", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "rounded-lg border border-border bg-card", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center justify-between border-b border-border px-4 py-2.5", children: [
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "rounded-lg border border-border bg-card", children: [
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "max-h-[340px] overflow-auto", children: /* @__PURE__ */ jsxRuntimeExports.jsxs(Table, { children: [
          ...
          /* @__PURE__ */ jsxRuntimeExports.jsx(TableBody, { children: apiKeys.slice(0, 10).map((k) => /* @__PURE__ */ jsxRuntimeExports.jsxs(TableRow, { className: "border-border text-xs", children: [
            ...
            /* @__PURE__ */ jsxRuntimeExports.jsx(TableCell, { className: "py-1.5 font-mono text-[11px] tabular-nums", children: k.requests_24h }),
            /* @__PURE__ */ jsxRuntimeExports.jsx(TableCell, { className: "py-1.5 font-mono text-[11px] text-muted-foreground", children: k.expires_at ? formatDistanceToNow(new Date(k.expires_at), { addSuffix: true }) : "Never" })
          ] }, k.id)) })
        ] }) })
      ] })
    ] }),
...
function SREDashboard() {
  ...
  const [slos, setSlos] = reactExports.useState([]);
  const [loading, setLoading] = reactExports.useState(true);
  reactExports.useEffect(() => {
    async function load() {
      setLoading(true);
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex flex-col", children: [
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "grid grid-cols-2 gap-3 px-6 py-4 md:grid-cols-4 xl:grid-cols-8", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx(MetricCard, { label: "Open incidents", value: metrics?.openIncidents ?? "—", series: generateTimeSeries(20, 3, 1), trend: -14.2, trendInverted: true, status: "warning" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx(MetricCard, { label: "SEV1 incidents", value: metrics?.sev1Incidents ?? "—", series: generateTimeSeries(20, 1, 0), trend: 0, status: metrics?.sev1Incidents ? "critical" : "success" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx(MetricCard, { label: "Error budget", value: `${metrics?.avgErrorBudget ?? 100}%`, series: generateTimeSeries(20, 82, 5), trend: -2.1, trendInverted: true, status: metrics?.avgErrorBudget && metrics.avgErrorBudget < 50 ? "error" : "warning" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx(MetricCard, { label: "Deploys 24h", value: metrics?.deploys24h ?? "—", series: generateTimeSeries(20, 14, 3), trend: 8.4, status: "info" }),
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsx(MetricCard, { label: "At-risk SLOs", value: slos.filter((s) => s.status === "at_risk").length, series: generateTimeSeries(20, 2, 1), trend: 0, status: "warning" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx(MetricCard, { label: "Breached SLOs", value: slos.filter((s) => s.status === "breached").length, series: generateTimeSeries(20, 0, 0), trend: 0, status: slos.some((s) => s.status === "breached") ? "error" : "success" }),
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "rounded-lg border border-border bg-card", children: [
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "max-h-[340px] overflow-auto", children: /* @__PURE__ */ jsxRuntimeExports.jsxs(Table, { children: [
          ...
          /* @__PURE__ */ jsxRuntimeExports.jsx(TableBody, { children: incidents.filter((i) => i.status !== "resolved").slice(0, 8).map((inc) => /* @__PURE__ */ jsxRuntimeExports.jsxs(TableRow, { className: "cursor-pointer border-border text-xs hover:bg-accent/40", children: [
            ...
            /* @__PURE__ */ jsxRuntimeExports.jsx(TableCell, { className: "py-1.5 font-mono text-[11px] text-muted-foreground", children: formatDistanceToNow(new Date(inc.opened_at), { addSuffix: true }) })
          ] }, inc.id)) })
        ] }) })
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "rounded-lg border border-border bg-card", children: [
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "max-h-[340px] overflow-auto", children: /* @__PURE__ */ jsxRuntimeExports.jsxs(Table, { children: [
          ...
          /* @__PURE__ */ jsxRuntimeExports.jsx(TableBody, { children: deployments.map((d) => /* @__PURE__ */ jsxRuntimeExports.jsxs(TableRow, { className: "border-border text-xs", children: [
            ...
            /* @__PURE__ */ jsxRuntimeExports.jsx(TableCell, { className: "py-1.5 font-mono text-[11px] text-muted-foreground", children: formatDistanceToNow(new Date(d.started_at), { addSuffix: true }) })
          ] }, d.id)) })
        ] }) })
      ] })
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "px-6 py-3", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "rounded-lg border border-border bg-card", children: [
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 p-4", children: slos.slice(0, 6).map((slo) => /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "rounded-md border border-border p-3", children: [
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "mt-2 h-2 overflow-hidden rounded-full bg-muted", children: /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: cn("h-full rounded-full transition-all", slo.budget_remaining > 50 ? "bg-success" : slo.budget_remaining > 20 ? "bg-warning" : "bg-destructive"), style: { width: `${slo.budget_remaining}%` } }) }),
...
function DeveloperDashboard() {
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex flex-col", children: [
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "grid grid-cols-2 gap-3 px-6 py-4 md:grid-cols-4 xl:grid-cols-8", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx(MetricCard, { label: "Your services", value: metrics?.serviceCount ?? "—", series: generateTimeSeries(20, 6, 1), trend: 0, status: "info" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx(MetricCard, { label: "Open issues", value: metrics?.openIssues ?? "—", series: generateTimeSeries(20, 2, 1), trend: -12, trendInverted: true, status: "warning" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx(MetricCard, { label: "p95 latency", value: `${metrics?.avgP95 ?? 0}ms`, series: generateTimeSeries(20, 142, 20), trend: -3.1, trendInverted: true, status: "info" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx(MetricCard, { label: "Error rate", value: `${metrics?.avgErrorRate ?? 0}%`, series: generateTimeSeries(20, 1.24, 0.4), trend: -8.2, trendInverted: true, status: "success" }),
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "rounded-lg border border-border bg-card", children: [
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "max-h-[300px] overflow-auto", children: /* @__PURE__ */ jsxRuntimeExports.jsxs(Table, { children: [
          ...
          /* @__PURE__ */ jsxRuntimeExports.jsx(TableBody, { children: events.slice(0, 15).map((e) => /* @__PURE__ */ jsxRuntimeExports.jsxs(TableRow, { className: "cursor-pointer border-border text-xs hover:bg-accent/40", onClick: () => setSelectedEvent(e), children: [
            ...
            /* @__PURE__ */ jsxRuntimeExports.jsx(TableCell, { className: "py-1.5 font-mono text-[11px]", children: e.event_type }),
            ...
            /* @__PURE__ */ jsxRuntimeExports.jsxs(TableCell, { className: cn("py-1.5 text-right font-mono text-[11px] tabular-nums", e.latency_ms > 300 && "text-warning"), children: [
              e.latency_ms,
              "ms"
            ] })
          ] }, e.id)) })
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsx(Dialog, { open: !!selectedEvent, onOpenChange: (o) => !o && setSelectedEvent(null), children: /* @__PURE__ */ jsxRuntimeExports.jsx(DialogContent, { className: "max-w-2xl", children: selectedEvent && /* @__PURE__ */ jsxRuntimeExports.jsxs(jsxRuntimeExports.Fragment, { children: [
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsxs(Tabs, { defaultValue: "payload", className: "mt-2", children: [
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsx(TabsContent, { value: "context", children: /* @__PURE__ */ jsxRuntimeExports.jsx("pre", { className: "max-h-64 overflow-auto rounded-md bg-background p-3 font-mono text-xs", children: JSON.stringify({ queue: selectedEvent.queue, worker: selectedEvent.worker, retries: selectedEvent.retries, latency_ms: selectedEvent.latency_ms }, null, 2) }) })
      ] })
    ] }) }) })
  ] });
}
...
function ViewerDashboard() {
  ...
  const [services, setServices] = reactExports.useState([]);
  const [alerts, setAlerts] = reactExports.useState([]);
  const [mlInsights, setMLInsights] = reactExports.useState([]);
  reactExports.useEffect(() => {
    async function load() {
  ...
  const serviceDist = services.reduce((acc, s) => {
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex flex-col", children: [
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "grid grid-cols-2 gap-3 px-6 py-4 md:grid-cols-4 xl:grid-cols-8", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx(MetricCard, { label: "System uptime", value: `${metrics?.uptime ?? 99.9}%`, series: generateTimeSeries(20, 99.9, 8e-3), trend: 0.01, status: "success" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx(MetricCard, { label: "Healthy services", value: `${metrics?.healthyServices ?? 0}/${metrics?.totalServices ?? 0}`, series: generateTimeSeries(20, 30, 2), trend: 0.5, status: "success" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx(MetricCard, { label: "Active alerts", value: metrics?.activeAlerts ?? 0, series: generateTimeSeries(20, 4, 2), trend: 0, status: "warning" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx(MetricCard, { label: "Events / sec", value: metrics?.eventsPerSec ?? 0, series: generateTimeSeries(20, 847, 80), trend: 4.2, status: "info", variant: "area" }),
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsx(MetricCard, { label: "Error rate", value: `${metrics?.uptime ? (100 - metrics.uptime).toFixed(2) : 0.1}%`, series: generateTimeSeries(20, 1.24, 0.4), trend: -8, trendInverted: true, status: "success" }),

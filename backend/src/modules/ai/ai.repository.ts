...
import type { ListQuery } from './ai.schema.js';

export const aiRepository = {
  async findMany(orgId: string, q: ListQuery) {
    const { skip, take } = paginate(q);
    const delegate = (prisma as Record<string, { findMany: Function; count: Function }>).aiInsight;
    const where = { orgId };
  ...
  async findById(orgId: string, id: string) {
    const delegate = (prisma as Record<string, { findFirst: Function }>).aiInsight;
